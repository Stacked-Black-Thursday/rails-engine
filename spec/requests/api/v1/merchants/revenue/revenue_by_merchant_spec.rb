  ...
  describe 'merchants with the most revenue' do
    describe 'happy path' do
      it "10 top merchants when with a quantity of 10" do
        quantity = 10
        get "/api/v1/revenue/merchants/most_revenue?quantity=#{quantity}"
        ...
        expect(json[:data].first[:id]).to eq(@merchant6.id.to_s)
        ...
        expect(json[:data].first[:attributes][:revenue]).to eq(20770.0)
        expect(json[:data].second[:attributes][:revenue]).to eq(4000.0)
      end
      ...
      it "all merchants if quantity is too large" do
        ...
        expect(json[:data].first[:attributes][:revenue]).to be_a(Float)
        expect(json[:data].first[:attributes][:revenue]).to eq(20770.0)
        expect(json[:data].last[:attributes][:revenue]).to eq(100.0)
      end
    end

    describe 'sad path' do
      it "returns an error when the quantity is not provided" do
        get "/api/v1/revenue/merchants/most_revenue"
